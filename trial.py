...
with db:
    ...
    print("\n=========== Serializers ===========\n")
    users1 = db.get_users_by_first_and_last_name(first_name="Adrian", last_name="Tamas")
    print(f"List of users by name {json.dumps(users1[0].serialize())}")

    book_by_name = db.get_books_by_name("Nightflyers")
    print(f"Book by exact name {json.dumps(book_by_name.serialize())}")
    ...
    row = db.update_user_with_entity(user.user_id, user)
    print(f"Nr of edited rows: {row}")
    new_user = db.get_users_by_id(user.user_id)
    print(f"Edited User: {new_user}")